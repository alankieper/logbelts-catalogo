...
function busquedaDespiece(marca, modelo) {
  return {
    label: `Buscar despiece / IPL de ${marca} ${modelo}`,
    url: 'https://www.google.com/search?q=' + encodeURIComponent(`${marca} ${modelo} despiece IPL parts diagram`.trim()),
  };
}
...
export async function despiecesDeProducto(codigo, compat) {
  ...
  const oficiales = norm.map((x) => ({
    ...
    diagramas: busquedaDespiece(x.marca, x.modelo),
  }));
