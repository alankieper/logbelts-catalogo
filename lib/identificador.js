...
function shortlist(productos, info, n = 10) {
  ...
  const top = scored.filter((x) => x.s > 0).slice(0, n * 3);
  const conFoto = top.filter((x) => x.p.foto).slice(0, n);
  const sinFoto = top.filter((x) => !x.p.foto).slice(0, 6);
...
[{"codigo":"...","confianza":<0-100>,"motivo":"<1 frase: por qué coincide (forma, medida, marca, cantidad de agujeros) o por qué no>"}]
Rankeá TODOS los candidatos de la lista (aunque la confianza sea baja) — el que vende decide.
No inventes códigos: usá sólo los de la lista.`,
  });

  let rank = [];
  try {
    const s2 = await client.messages.create({ model: MODEL, max_tokens: 1100, messages: [{ role: 'user', content }] });
  ...
  let resultados = (Array.isArray(rank) ? rank : [])
    .map((r) => ({ codigo: r.codigo, confianza: Number(r.confianza) || 0, motivo: r.motivo || '', producto: byCode.get(r.codigo) }))
    .filter((r) => r.producto)
    .sort((a, b) => b.confianza - a.confianza)
    .slice(0, 8);

  // fallback: si la IA no rankeó nada, mostrar igual la lista corta como "posibles"
  if (!resultados.length) {
    resultados = cand.slice(0, 6).map((p) => ({
      codigo: p.codigo,
      confianza: null,
      motivo: `Candidato por tipo (${info.tipo || 'pieza'}) — revisar contra la foto`,
      producto: p,
    }));
  }
