...
/** ¿"a" podría ser "b" tipeado con algún error? Tolerancia según el largo de la palabra. */
function esTypoDe(a, b) {
  if (Math.abs(a.length - b.length) > 2) return false;
  const umbral = a.length <= 4 ? 1 : a.length <= 8 ? 2 : 3;
  return distancia(a, b) <= umbral;
}
...
export async function buscar(q, limite = 0) {
  ...
  for (const p of c.productos) {
    ...
    if (code === raw) {
      ...
      // Nada calzó exacto: probamos si cada término podría ser un error de tipeo
      // de alguna palabra real de la ficha (ej. "carburdor" -> "carburador").
      const hayPalabras = hay.split(/[^a-z0-9ñ]+/).filter((w) => w.length >= 3);
      const todasCalzan = terms.every((t) => hay.includes(t) || hayPalabras.some((w) => esTypoDe(t, w)));
      if (todasCalzan) {
        score = 25;
        motivo = 'Coincidencia aproximada (revisá la ortografía)';
      }
    }
